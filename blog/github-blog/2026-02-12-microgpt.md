这是microgpt的简要指南——一个200行纯Python文件，无依赖，训练和推理GPT。包含完整算法：数据集、分词器、自动求导、GPT-2类似架构、Adam优化器、训练循环、推理循环。

**数据集：** 32,000个名字。**分词器：** 字符级，27个token（a-z + BOS）。**自动求导：** 从头实现的Value类。**参数：** 4,192个。**架构：** GPT-2简化版——嵌入、注意力、MLP、残差连接。**训练：** Adam优化器，1000步。**推理：** 从BOS开始采样，生成新名字。

**与ChatGPT的关系：** microgpt包含了算法本质，但生产级LLM需要万亿级token、BPE分词、GPU张量操作、数千亿参数、大规模分布式训练和后训练（SFT+RL）。
