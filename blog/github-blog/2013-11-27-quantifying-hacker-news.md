...
我设置了一个非常简单的 Python 脚本，每隔一分钟抓取 HN 的**首页**和**新故事页面**。一天的数据从凌晨 4 点（PST）开始，到第二天凌晨 4 点结束。HTML 文件被压缩保存为 gzipped pickle 格式，一天的数据大约占用 10MB。我中途有几次关闭了机器几天，所以数据中存在一些空缺，但最终我们得到了从 8 月 22 日到 10 月 30 日期间的 47 天数据。
