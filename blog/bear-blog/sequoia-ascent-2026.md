...
Karpathy 在 Sequoia Ascent 2026 上做了一个炉边谈话。他让 LLM 基于他最近的博客和推文生成了谈话摘要和清理过的文字稿。

核心观点：

**1. 2025 年 12 月是 Agent 转折点**。从那时起，AI 生成的代码块更大了、更可靠了。编程的单位从"写一行行代码"变成了"委托更大的宏操作"——实现功能、重构模块、研究库等。程序员越来越像 Agent 的编排者。

**2. 软件 3.0：上下文窗口就是新程序**。软件 1.0 是人写代码，2.0 是训练神经网络，3.0 是通过提示词、上下文、工具、示例和指令来编程 LLM。

**3. MenuGen 的启示**。传统上做一个菜单配图 App 需要前端、后端、API、部署等全套流程，但在软件 3.0 范式下，直接给多模态模型一张菜单照片，让它直接在照片上渲染菜品图片就行了——大部分"App"代码消失了。

**4. 新机会不只是更快的编程**。LLM 能自动化以前无法编程的信息处理任务，比如自动从原始文档中增量编译成持久的 Markdown 知识库。

**5. 可验证性**：传统软件自动化"你能指定的事情"，LLM 自动化"你能验证的事情"。可验证的领域（数学、编程、测试）进步最快。

**6. 锯齿智能**：模型能力不仅取决于可验证性，还取决于训练时的注意力分配。创始人的问题是：你的任务是否在模型的"轨道"上？

**7. Vibe Coding vs Agentic Engineering**：Vibe Coding 提高下限（人人都能写代码），Agentic Engineering 提高上限（专业人士协调 Agent 保持代码质量）。

**8. 招聘应改变**：传统的编程题不再合适，应该考察候选人用 Agent 构建完整项目的能力。

**9. 创始人应寻找有价值的可验证环境**。如果某个领域有可验证的结构但大实验室还没投入，那就是创业机会。

**10. Agent 原生基础设施**：产品需要为 Agent 设计界面——Markdown 文档、CLI、API、MCP 服务器等。

**11. 幽灵而非动物**：不要把 LLM 当动物看待，它们没有生物驱动力。它们是统计数据模拟。

**12. 教育：可以外包思考，但不能外包理解**。你需要理解才能有效指挥 Agent。
