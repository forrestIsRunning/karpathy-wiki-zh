...
Karpathy 分享了他多年来最有效的笔记方法——"追加与回顾笔记"。

**数据结构**：只在 Apple Notes 中维护一个笔记，命名为"notes"。只有一个笔记意味着搜索 (CTRL+F) 非常简单。

**追加**：任何时候有想法或待办事项，就追加到笔记顶部。不需要标记日期、链接等元数据。唯一例外是用 "watch:"、"listen:"、"read:" 等标签方便搜索。

**回顾**：新内容加在顶部，旧内容自然下沉。定期向下滚动浏览。如果发现值得关注的内容，就复制粘贴到顶部。有时合并、处理、分组相关的笔记。很少删除。不需要重复关注的笔记会自然沉下去。

这种做法的好处：记下来就能立刻清空工作记忆，专注于当前任务。相信之后回顾时会重新处理。

他的笔记经过几年已经变得很大。偶尔翻看以前的想法很有意思——有些经不起反复推敲，会越沉越深；有些很久以前的想法在新的光线下突然变得相关了。
